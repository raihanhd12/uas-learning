...
<div class="iphone">
    <a href="/memberships">Back</a>
    <header class="header">
    
      <h1>  Checkout Pesanan</h1>
    </header>
  
    <form  method="POST" action="/checkout" class="form"  enctype="multipart/form-data">
      @csrf
      <div>
        ...
        <div class="card">
          <address>
            {{ $membership->title }}
            <hr>
            {{ $membership->bulan }} Month Membership
          </address>
        </div>
      </div>  
      <input type="hidden" id="username" name="username" value="{{ auth()->user()->username }}">  
      <input type="hidden" id="membership" name="membership" value="{{ $membership->title }}">       
      <fieldset>
      ...
      <div>
          ...
          <table>
              <tbody>
                  <tr>
                      <td>{{ $membership->bulan }} Month Membership (incl. tax)</td>
                      <td align="right">Rp. {{ $membership->harga }}</td>
                    </tr>
                    <tr>
                        <td></td>
              <td></td>
    ...
    <input type="file" id="image" name="image" onchange="readFoto(event)" required>   
